...
source("./code_functions/realdata_RI.R")
...
mianresult_list <- list(
    "SCL-Linear" = realdata.ri.SCL.linear,
    "SCL-Gaussian" = realdata.ri.SCL.gaussian,
    "Wangs method" = realdata.ri.wang,
    "QIQ-learning" = realdata.ri.QIQ
)
...
result_table_compact[["RI for tau=0.25"]] <- character(n_methods)
result_table_compact[["RI for tau=0.50"]] <- character(n_methods)
result_table_compact[["RI for tau=0.75"]] <- character(n_methods)
...
for (j in 1:n_methods) {
  ...
  result_table_compact[j, "RI for tau=0.25"] <- sprintf("%.3f (%.3f)", means[1], sds[1])
  result_table_compact[j, "RI for tau=0.50"] <- sprintf("%.3f (%.3f)", means[2], sds[2])
  result_table_compact[j, "RI for tau=0.75"] <- sprintf("%.3f (%.3f)", means[3], sds[3])
}

cat(kable(result_table_compact, format = "simple", caption = "Table 3: Mean of RI for different methods on real data"), sep = "\n", file = "./output/Table3.txt")