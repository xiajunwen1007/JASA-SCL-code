...
simulation <- function(l, n, d, err, base, base.test, x.test, eps.test, opt.q.25, opt.q.50, q.25, q.50) {
  ...
  # true value function
  v.SCL.linear.25 <- quantile(eval(parse(text = base.test)) + xtest.treat.SCL.linear.25  * (x.test[, 2] + x.test[, 3]) + exp(-2-0.5*x.test[,2]-0.5*x.test[,3]+xtest.treat.SCL.linear.25*(2+x.test[,2]+x.test[,3])) * eps.test, 0.25)
  v.SCL.linear.50 <- quantile(eval(parse(text = base.test)) + xtest.treat.SCL.linear.50  * (x.test[, 2] + x.test[, 3]) + exp(-2-0.5*x.test[,2]-0.5*x.test[,3]+xtest.treat.SCL.linear.50*(2+x.test[,2]+x.test[,3])) * eps.test, 0.50)

  v.SCL.gaussian.25 <- quantile(eval(parse(text = base.test)) + xtest.treat.SCL.gaussian.25  * (x.test[, 2] + x.test[, 3]) + exp(-2-0.5*x.test[,2]-0.5*x.test[,3]+xtest.treat.SCL.gaussian.25*(2+x.test[,2]+x.test[,3])) * eps.test, 0.25)
  v.SCL.gaussian.50 <- quantile(eval(parse(text = base.test)) + xtest.treat.SCL.gaussian.50  * (x.test[, 2] + x.test[, 3]) + exp(-2-0.5*x.test[,2]-0.5*x.test[,3]+xtest.treat.SCL.gaussian.50*(2+x.test[,2]+x.test[,3])) * eps.test, 0.50)
  v.QIQ.25 <- quantile(eval(parse(text = base.test)) + xtest.treat.QIQ.25  * (x.test[, 2] + x.test[, 3]) + exp(-2-0.5*x.test[,2]-0.5*x.test[,3]+xtest.treat.QIQ.25*(2+x.test[,2]+x.test[,3])) * eps.test, 0.25)
  v.QIQ.50 <- quantile(eval(parse(text = base.test)) + xtest.treat.QIQ.50  * (x.test[, 2] + x.test[, 3]) + exp(-2-0.5*x.test[,2]-0.5*x.test[,3]+xtest.treat.QIQ.50*(2+x.test[,2]+x.test[,3])) * eps.test, 0.50)

  v.wang.25 <- quantile(eval(parse(text = base.test)) + xtest.treat.wang.25 * (x.test[, 2] + x.test[, 3]) + exp(-2-0.5*x.test[,2]-0.5*x.test[,3]+xtest.treat.wang.25*(2+x.test[,2]+x.test[,3])) * eps.test, 0.25)
  v.wang.50 <- quantile(eval(parse(text = base.test)) + xtest.treat.wang.50  * (x.test[, 2] + x.test[, 3]) + exp(-2-0.5*x.test[,2]-0.5*x.test[,3]+xtest.treat.wang.50*(2+x.test[,2]+x.test[,3])) * eps.test, 0.50)

  v.SCL.linear.25.hard <- quantile(eval(parse(text = base.test)) + xtest.treat.SCL.linear.25.hard * (x.test[, 2] + x.test[, 3]) + exp(-2-0.5*x.test[,2]-0.5*x.test[,3]+xtest.treat.SCL.linear.25.hard*(2+x.test[,2]+x.test[,3])) * eps.test, 0.25)
  v.SCL.linear.50.hard <- quantile(eval(parse(text = base.test)) + xtest.treat.SCL.linear.50.hard * (x.test[, 2] + x.test[, 3]) + exp(-2-0.5*x.test[,2]-0.5*x.test[,3]+xtest.treat.SCL.linear.50.hard*(2+x.test[,2]+x.test[,3])) * eps.test, 0.50)
  v.SCL.gaussian.25.hard <- quantile(eval(parse(text = base.test)) + xtest.treat.SCL.gaussian.25.hard * (x.test[, 2] + x.test[, 3]) + exp(-2-0.5*x.test[,2]-0.5*x.test[,3]+xtest.treat.SCL.gaussian.25.hard*(2+x.test[,2]+x.test[,3])) * eps.test, 0.25)
  v.SCL.gaussian.50.hard <- quantile(eval(parse(text = base.test)) + xtest.treat.SCL.gaussian.50.hard * (x.test[, 2] + x.test[, 3]) + exp(-2-0.5*x.test[,2]-0.5*x.test[,3]+xtest.treat.SCL.gaussian.50.hard*(2+x.test[,2]+x.test[,3])) * eps.test, 0.50)
  ...
  true.25 <- as.numeric((q.25 - eval(parse(text = base)))/exp(-2-0.5*x[,2]-0.5*x[,3])-((q.50 - eval(parse(text = base)) - x[,2]-x[,3])/exp(0.5*x[,2]+0.5*x[,3]))>0)
  true.50 <- as.numeric((q.50 - eval(parse(text = base)))/exp(-2-0.5*x[,2]-0.5*x[,3])-((q.50 - eval(parse(text = base)) - x[,2]-x[,3])/exp(0.5*x[,2]+0.5*x[,3]))>0)
...
y <- eval(parse(text = case2.base)) +   a * (x[, 2] + x[, 3]) + exp(-2-0.5*x[,2]-0.5*x[,3]+a*(2+x[,1]+x[,2])) * eps
...
# parallel computing
system.time({
  res <- parLapply(cl, 1:times, simulation, n = n, d= d, err = case2.err, base = case2.base, base.test = case2.base.test, x.test = x, eps.test = eps, opt.q.25 = case2.opt.q.25, opt.q.50 = case2.opt.q.50, q.25 = case2.q.25,q.50 = case2.q.50)
})
stopCluster(cl)

# extract results
# value funtion and misclassification rates
case2.mianresult.SCL.linear <- array(0, dim = c(times, 4))
case2.mianresult.SCL.gaussian <- array(0, dim = c(times, 4))
case2.mianresult.QIQ <- array(0, dim = c(times, 4))
case2.mianresult.wang <- array(0, dim = c(times, 4))
case2.mianresult.SCL.linear.hard <- array(0, dim = c(times, 4))
case2.mianresult.SCL.gaussian.hard <- array(0, dim = c(times, 4))

#computation time
case2.time.SCL.linear <- array(0, dim = c(times, 2))
case2.time.SCL.gaussian <- array(0, dim = c(times, 2))
case2.time.QIQ <- array(0, dim = c(times, 2))
case2.time.wang <- array(0, dim = c(times, 2))

#bias of the estimated optimal value
case2.q.SCL.linear <- array(0, dim = c(times, 2))
case2.q.SCL.gaussian <- array(0, dim = c(times, 2))
case2.q.QIQ <- array(0, dim = c(times, 2))
case2.q.wang <- array(0, dim = c(times, 2))

#estimated value function
case2.hatv.SCL.linear <- array(0, dim = c(times, 2))
case2.hatv.SCL.gaussian <- array(0, dim = c(times, 2))
case2.hatv.QIQ <- array(0, dim = c(times, 2))
case2.hatv.wang <- array(0, dim = c(times, 2))

case2.hatv.true <- array(0, dim = c(times, 2))

# quantile optimal treatment on a grid for plotting purpose, which is only feasible when d=2.
if(d==2){
  case2.xgrid.treat.SCL.linear.25 <- array(0, dim = c(times, 500*500))
  case2.xgrid.treat.SCL.gaussian.25 <- array(0, dim = c(times, 500*500))
  case2.xgrid.treat.QIQ.25 <- array(0, dim = c(times, 500*500))
  case2.xgrid.treat.wang.25 <- array(0, dim = c(times, 500*500))

  case2.xgrid.treat.SCL.linear.50 <- array(0, dim = c(times, 500*500))
  case2.xgrid.treat.SCL.gaussian.50 <- array(0, dim = c(times, 500*500))
  case2.xgrid.treat.QIQ.50 <- array(0, dim = c(times, 500*500))
  case2.xgrid.treat.wang.50 <- array(0, dim = c(times, 500*500))
}

for (j in 1:times) {
  case2.mianresult.SCL.linear[j, ] <- res[[j]]$SCL.linear
  case2.mianresult.SCL.gaussian[j, ] <- res[[j]]$SCL.gaussian
  case2.mianresult.QIQ[j, ] <- res[[j]]$QIQ
  case2.mianresult.wang[j, ] <- res[[j]]$wang
  case2.mianresult.SCL.linear.hard[j, ] <- res[[j]]$SCL.linear.hard
  case2.mianresult.SCL.gaussian.hard[j, ] <- res[[j]]$SCL.gaussian.hard
  
  case2.time.SCL.linear[j, ] <- res[[j]]$time.SCL.linear
  case2.time.SCL.gaussian[j, ] <- res[[j]]$time.SCL.gaussian
  case2.time.QIQ[j, ] <- res[[j]]$time.QIQ
  case2.time.wang[j, ] <- res[[j]]$time.wang
 
  case2.q.SCL.linear[j, ] <- res[[j]]$q.SCL.linear
  case2.q.SCL.gaussian[j, ] <- res[[j]]$q.SCL.gaussian
  case2.q.QIQ[j, ] <- res[[j]]$q.QIQ
  case2.q.wang[j, ] <- res[[j]]$q.wang

  case2.hatv.SCL.linear[j, ] <- res[[j]]$hatv.SCL.linear
  case2.hatv.SCL.gaussian[j, ] <- res[[j]]$hatv.SCL.gaussian
  case2.hatv.QIQ[j, ] <- res[[j]]$hatv.QIQ
  case2.hatv.wang[j, ] <- res[[j]]$hatv.wang
  case2.hatv.true[j, ] <- res[[j]]$hatv.true
  
  if(d==2){
    case2.xgrid.treat.SCL.linear.25[j, ] <- res[[j]]$xgrid.treat.SCL.linear.25
    case2.xgrid.treat.SCL.gaussian.25[j, ] <- res[[j]]$xgrid.treat.SCL.gaussian.25
    case2.xgrid.treat.QIQ.25[j, ] <- res[[j]]$xgrid.treat.QIQ.25
    case2.xgrid.treat.wang.25[j, ] <- res[[j]]$xgrid.treat.wang.25

    case2.xgrid.treat.SCL.linear.50[j, ] <- res[[j]]$xgrid.treat.SCL.linear.50
    case2.xgrid.treat.SCL.gaussian.50[j, ] <- res[[j]]$xgrid.treat.SCL.gaussian.50
    case2.xgrid.treat.QIQ.50[j, ] <- res[[j]]$xgrid.treat.QIQ.50
    case2.xgrid.treat.wang.50[j, ] <- res[[j]]$xgrid.treat.wang.50
  }
}